...
CREDENTIALS_FILE = 'iot-nhom6.json'   # service account json (đã có)
SHEET_NAME = 'iot-chamcongtudong'
WORKSHEET_NAME = 'DANHSACH'

# Google Drive Config
GOOGLE_DRIVE_FOLDER_ID = '0APc81r36Z77dUk9PVA'  # <-- Thay bằng folder ID của bạn

# Thingsboard Config (giữ nguyên nếu cần)
...
def upload_to_drive(image_path):
    try:
        ...
        file_metadata = {
            ...
            'parents': [GOOGLE_DRIVE_FOLDER_ID]   # Shared Drive ID
        }
...
# =======================
# CHẤM CÔNG (KHÔNG GHI LINK ẢNH)
# =======================
def log_attendance(sheet, person_id, person_name, img_path):
    """
    Ghi ID, Name, Date, Time, Status vào Google Sheets.
    Đồng thời upload ảnh lên Google Drive (KHÔNG ghi link vào Sheets).
    """
    if sheet is None:
        return False
    ...
    try:
            ...
            for row in all_values[1:]:
                if len(row) >= 3:
                    if str(row[0]).strip() == str(person_id) and row[2].strip() == date_str:
                        ...
                        # Vẫn upload ảnh (không ghi link vào sheet)
        ...
        # Ghi Google Sheets (không kèm link ảnh)
